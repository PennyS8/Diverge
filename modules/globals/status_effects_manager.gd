...
#
#@onready var yarn_line2d : Line2D
#var YARN_LENGTH = 128
...
#func _physics_process(delta: float) -> void:
	#yarn_line2d = StatusEffectsManager.get_node_or_null("YarnLine2D")
	...
	#var node_2_pos : Vector2 = tethered_nodes[1].global_position
		...
		#if yarn_line2d:
			#yarn_line2d.queue_free()
		#return # Don't update yarn_line2d if yarn has broken
	
	## Draw/update yarn_line2d
	#if !yarn_line2d:
		#create_yarn_line2d()
	#update_tethered_yarn()
...
#func create_yarn_line2d():
	#yarn_line2d = Line2D.new()
	#yarn_line2d.set_name("YarnLine2D")
	#yarn_line2d.default_color = Color(255, 255, 0, 0.5)
	#yarn_line2d.width = 2
	#yarn_line2d.z_index = 1
	#StatusEffectsManager.add_child(yarn_line2d)
#
#func update_tethered_yarn():
	#if !yarn_line2d:
		#return
	#
	#yarn_line2d.clear_points()
	#var tethered_nodes = get_tree().get_nodes_in_group("status_tethered")
	#
	#if tethered_nodes.size() == 2:
		#var node_1_pos = tethered_nodes[0].global_position
		#var node_2_pos = tethered_nodes[1].global_position
		#var p1 = yarn_line2d.to_local(node_1_pos)
		#var p2 = yarn_line2d.to_local(node_2_pos)
		#yarn_line2d.add_point(p1)
		#yarn_line2d.add_point(p2)
